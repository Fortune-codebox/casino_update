...
app.use(cors({
  origin: "*",  // or whatever port you're using
  credentials: true
}));
...
const database_config = constants.DATABASE_URL
// const uuidToSocketMap = new Map();
...
io.on('connection', function(socket) {
  ...
  // if (uuid) {
  //   uuidToSocketMap.set(uuid, socket.id);
  //   console.log(`[✅] UUID ${uuid} is now linked to socket ${socket.id}`);
  // }
  ...
  socket.on('signin_send', async (data) => {
    ...
    database_config.name = "db01"
		const result = await database(database_config);
    if(result){
      users_array = result
      let user_found = users_array.filter(function(x){
        return (x.user === data.user || x.email === data.email) && decrypt(x.password) === data.pass
      })
  ...
  socket.on('signup_send', async (data) => {
    ...
    database_config.sql = 'SELECT * FROM casino_users WHERE email = "' + data.email + '"'
    database_config.name = "db02"
		const user = await database(database_config)
    if(user && user.length == 0){
      ...
      try{
        database_config.sql = `INSERT INTO casino_users (
          uuid,
          "user",
          email,
          password,
        ...
        const res = await database(database_config)
        io.to(socket.id).emit('signup_read', {exists: false, obj: {}})
      } catch(e){
      ...
      let array = user.length && user.filter(function(x){ //check if there already is an email with same username
